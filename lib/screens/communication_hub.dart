...
class _CommunicationHubState extends State<CommunicationHub> {
  ...
  void initState() {
    ...
    messageCardService =
        MessageCardClickCountService(profileId: widget.profileId);
  ...
  Widget build(BuildContext context) {
    /* Wrap the Scaffold in PopScope setting canPop to false,
       disabling the users ability to swipe back to the previous screen*/
    return PopScope(
      canPop: false,
      child: Scaffold(
        appBar: AppBar(
          ...
          actions: <Widget>[
            ...
            LongPressButton(
              /*pop back to admin home on successful long press also pass
            * the press time for the button*/
              onLongPressCompleted: Navigator.of(context).pop,
              pressTime: 8,
            ),
          ],
        ),
